...
int main(int argc, char *argv[]) {
  if (argc < 3) {
    std::cerr << "Not enough arguments: Provide directory and body_names" << std::endl;
    return 0;
  }
  ...
  auto color_camera_ptr{
      std::make_shared<icg::RealSenseColorCamera>("realsense_color")};
      //std::make_shared<icg::AzureKinectColorCamera>("azure_kinect_color")};
  auto depth_camera_ptr{
      std::make_shared<icg::RealSenseDepthCamera>("realsense_depth")};
  ...
  for (const auto body_name : body_names) {
    ...
    //// Added by Nicklas
    auto publisher_ptr{
        std::make_shared<icg::PosePublisher>(body_name + "publisher")};
        //std::make_shared<icg::Publisher>(body_name + "publisher", metafile_path)};
  }
