...
from ove6d.utility import cam_control
...
def main(args):
    ...
    color_frames = np.zeros((frames, cfg.RENDER_HEIGHT, cfg.RENDER_WIDTH, 3), dtype=np.uint8)
    depth_frames = np.zeros((frames, cfg.RENDER_HEIGHT, cfg.RENDER_WIDTH), dtype=np.float32)
    ...
    try:
        ...
        with h5py.File("data/rs_records.hdf5", "a") as f:
        ...
        print(f"Saving {duration*frames} frames of shape {depth_frames.shape} and {color_frames.shape} to {filename}")
